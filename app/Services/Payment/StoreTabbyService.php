...
class StoreTabbyService
{
    ...
    public function createPaymentRequest(array $orderData, float $amount, array $customerData): array
    {
        try {
            ...
            if ($testCredentials) {
                ...
                } else if (!empty($orderData['user_id'])) {
    ...
    public function capturePayment(string $paymentId, float $amount = null): array
    {
        try {
            ...
            Log::error('Exception in Tabby payment capture', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    {
        try {
            Log::info('Refreshing Tabby payment status', ['payment_id' => $paymentId]);
            ...
            Log::info('Tabby payment status refresh response', [
                'endpoint' => $endpoint,
                'status' => $statusCode,
                'response' => $responseData
            ]);
            ...
            if (!$response->successful()) {
                if (config('services.tabby.is_sandbox')) {
                    Log::info('Tabby status refresh failed in sandbox mode, returning default success', [
                        'status' => $statusCode,
                        'response' => $responseData
                    ]);
            ...
            Log::error('Exception in Tabby payment status refresh', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
